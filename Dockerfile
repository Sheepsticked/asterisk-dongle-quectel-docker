...
RUN set -x \
	...
	&& apt-get install -y --no-install-recommends \
		...
		libasound2-dev sendemail \
		libnet-ssleay-perl libio-socket-ssl-perl \
		aptitude libedit-dev uuid-dev libjansson-dev libxml2-dev \
		libncurses-dev libnewt-dev libssl-dev python3-dev \
		libopus-dev libcurl4-openssl-dev libsqlite3-dev libgsm1-dev \
	&& rm -rf /var/lib/apt/lists/* \
# Download asterisk src
	&& git clone --branch ${ASTERISK_VERSION} --single-branch --depth 1 \
		https://github.com/asterisk/asterisk.git /usr/local/src/asterisk \
...
# Install chan-quectel
	...
	&& ./bootstrap \
	&& ./configure --with-astversion=${ASTERISK_VERSION_QUECTEL} \
	&& make -j"$(nproc)" all \
	&& make install \
